...
from cell.types import ToolSpec
...
class DiagnosticianAgent(Agent):
    async def invoke(self, input: AgentInput) -> AgentOutput:
        system_prompt = (
            ...
            "Analyze the blocker and choose one action: tool_spec, context_request, or escalate.\n"
            "If action is tool_spec, produce a complete ToolSpec with exact input/output schemas, at least "
            "3 behavioral test cases, and at least 2 edge cases.\n"
        ...
        if payload.get("action") == "tool_spec":
            payload["tool_spec"] = ToolSpec.model_validate(payload["tool_spec"]).model_dump(mode="json")
        elif payload.get("action") not in {"context_request", "escalate"}:
            return AgentOutput(
                status="error",
                payload={"error": f"Unsupported diagnostician action: {payload.get('action')!r}"},
                reasoning_trace=result.content,
                token_usage={"input": result.tokens_in, "output": result.tokens_out},
                model_id=result.model,
                latency_ms=result.latency_ms,
